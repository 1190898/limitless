<div align="center">

# limitless

**A Jujutsu Kaisen-inspired fan mod for Minecraft (Forge 1.20.1)**  
*一款基于《咒术回战》同人创作的 Minecraft 模组（Forge 1.20.1）*

[![Minecraft](https://img.shields.io/badge/Minecraft-1.20.1-brightgreen.svg)]()
[![Loader](https://img.shields.io/badge/Loader-Forge-orange.svg)]()
[![License](https://img.shields.io/badge/License-MIT-blue.svg)]()

[English](#english) | [简体中文](#简体中文)

</div>

---

<a name="english"></a>
## 📖 English

### Overview
Welcome to **limitless**! This mod brings iconic concepts from *Jujutsu Kaisen* into Minecraft 1.20.1 Forge. 

> ⚠️ **Disclaimer:** This is a **fan-made project**. The mechanics, balancing, and ability interactions have been adapted and modified for Minecraft gameplay, so **they do not 100% align with the original manga/anime canon**.

---

### ✨ Features & Techniques

* **Limitless (无下限)**  
  Manipulate space to keep incoming threats at bay. Bend distance itself to shield yourself from harm and unleash destructive spatial distortions.

* **Shrine (御厨子)**  
  Master precision cutting attacks. Carve through enemies alike with slicing techniques inspired by pure malevolence.

* **Light and Bloodlust (光与杀意)**  
  *(Original/Fan-made Concept)* A unique, high-octane combat technique balancing blinding radiance with raw, lethal intent.

* **Blood Manipulation (赤血操术)**  
  Control your own blood for diverse offensive and defensive utilities. Reinforce physical attributes, forge hardened blood weaponry, and fire piercing projectiles.

* **Adaptation (适应)**  
  A survival mechanic inspired by the Eight-Handled Sword Divergent Sila Divine General Mahoraga. Adapt to repeated incoming damage types, turning lethal threats into harmless scratches over prolonged combat.

* **Invisible Hand(无形之手) (Addon Mod)**  
  The ability to trade items, abilities, life, etc., and also trade special abilities with mysterious merchants, including special time-based abilities.  

* **Puppet(木偶)**  
  Complementing the "Invisible Hand," after unlocking, it allows control of one to three creatures with switchable perspectives.
---

### 📥 Requirements & Installation

1. Make sure you have **Minecraft Forge** installed for version **1.20.1** (Recommended latest stable build).
2. Requires **Java 17**.
3. Place the mod `.jar` file into your `.minecraft/mods` folder.
4. Launch the game and enjoy!

---

### 📜 Disclaimer
This is an unofficial, non-profit fan-made mod. *Jujutsu Kaisen* and all related trademarks belong to Gege Akutami / Shueisha / MAPPA. 

---

<a name="简体中文"></a>
## 📖 简体中文

### 模组简介
欢迎游玩 **limitless**！本模组基于《咒术回战》（Jujutsu Kaisen）的设定进行同人二次创作，适用于 Minecraft 1.20.1 Forge 平台。

> ⚠️ **说明与免责：** 本模组为**同人自制衍生作品**。为了兼顾 Minecraft 的玩法节奏与战斗数值平衡，模组内的技能机制、伤害判定与效果呈现**与原作设定并不完全一致**，请以游戏内实际体验为准。

---

### ✨ 核心内容与术式

* **无下限（Limitless）**  
  操控空间的极致防御与破坏。收束距离阻隔外界攻击，释放强大的空间扭曲摧毁敌人。

* **御厨子（Shrine）**  
  精密的斩击艺术。以凌厉的刀刃撕裂周围的生物，带来极致的收割与压迫感。

* **光与杀意（Light and Bloodlust）**  
  *(同人原创设计)* 兼具耀眼光芒与纯粹杀意的战斗流派，拥有独特的攻击节奏与高爆发机制。

* **赤血操术（Blood Manipulation）**  
  将血液化为武器。可进行近战硬化、血液流向强化身体机能，或凝聚高压血矢进行远距离穿透。

* **适应（Adaptation）**  
  灵感来源于“八握剑异戒神将魔虚罗”的防御机制。受到特定类型的伤害后会逐渐产生抗性并完成“适应”，让同一种攻击不再奏效。

* **无形之手（附属模组）**  
  交易物品、能力、生命等的能力，也能和神秘商人交易特殊能力，包括了特殊了时间系能力

* **木偶**  
  与“无形之手”相辅相成，解锁后可操控一到三个生物，可切换视角。

---

### 📥 安装前置与方法

1. 确保已安装对应 **1.20.1** 版本的 **Minecraft Forge**（建议使用最新稳定版）。
2. 需要 **Java 17** 运行环境。
3. 将下载好的模组 `.jar` 文件放入游戏目录下的 `.minecraft/mods` 文件夹内。
4. 启动游戏即可游玩。

---

### 📜 版权声明
本模组为非官方、非营利的同人衍生作品。《咒术回战》相关著作权及知识产权归芥见下下、集英社及 MAPPA 等原作者与版权方所有。
